...
public class MyPageProcess implements PageProcessor {
    ...
    public void process(Page page) {
        ...
        if(url.startsWith("http://blog.sina.com.cn/s/articlelist_1197161814_0_1.html"))
        {
            ...
            links.add(page.getHtml().xpath("//span[@class='atc_title']/a/@href").get());//page.getHtml().xpath("//span[@class='atc_title']/a/@href").all();
            System.out.println("----提取目录1：" + links.size() + ",list:"+JSON.toJSONString(links));
            page.addTargetRequests(links);
            //然后提取其他列表页地址
            links = page.getHtml().xpath("//ul[@class='SG_pages']/li/a/@href").all();
            System.out.println("----提取目录2：" + links.size() + ",list:"+JSON.toJSONString(links));
            page.addTargetRequests(links);
            page.setSkip(true);
            ...
            // 说明不是第一篇 李开复：母亲的十件礼物|悼文 ，需要重新 get 内容
